...
-- Index to quickly retrieve call history for a specific customer
CREATE INDEX idx_call_records_phone
ON call_records(customer_phone);

-- Index to speed up analytics queries grouped by intent
CREATE INDEX idx_call_records_intent
ON call_records(intent);

-- Index for efficiently retrieving the most recent calls
